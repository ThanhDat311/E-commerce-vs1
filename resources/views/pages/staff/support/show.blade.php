<x-staff-layout :pageTitle="'Ticket #'.$ticket->id" :breadcrumbs="['Staff' => route('staff.dashboard'), 'Tickets' => route('staff.support.index'), '#'.$ticket->id => '#']">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100vh-200px)]">
        ...
        <div class="lg:col-span-2 flex flex-col bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div class="p-4 bg-white border-t border-gray-200">
                <form action="{{ route('staff.support.reply', $ticket) }}" method="POST" enctype="multipart/form-data">
                    @csrf
        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
                ...
                <form action="{{ route('staff.support.update', $ticket) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <span class="text-sm font-medium text-gray-900 capitalize">{{ $ticket->priority }}</span>
                        </div>

                        <div>
                            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Assigned To</label>
                            @if($ticket->assigned_to)
                                <span class="text-sm font-medium text-gray-900">{{ $ticket->assignedTo->name }}</span>
                            @else
                                <span class="text-sm text-gray-400 italic">Unassigned</span>
                                <button type="submit" name="action" value="claim" class="ml-2 text-xs text-blue-600 hover:underline">Claim</button>
                            @endif
                        </div>
                    </div>
                </form>
