<x-admin-layout :pageTitle="'Risk Rule Management'" :breadcrumbs="['Admin' => route('admin.dashboard'), 'AI Dashboard' => route('admin.ai.dashboard.index'), 'Risk Rules' => route('admin.ai.risk-rules.index')]">
    ...
    <div x-data="{
        search: '',
        levelFilter: 'all',
        rules: {{ Js::from($rules->map(fn($r) => ['id' => $r->id, 'key' => $r->rule_key, 'desc' => $r->description, 'level' => $r->risk_level, 'active' => $r->is_active, 'weight' => $r->weight])) }},
        simulating: false,
        showSimModal: false,
        simRuleData: { key: '', currentWeight: 0, newWeight: 0 },
        simResult: null,
        openSimulateModal(rule) {
            this.simRuleData = { key: rule.key, currentWeight: rule.weight, newWeight: rule.weight };
            this.simResult = null;
        ...
        getRiskLevelInfo(level) {
            const colors = {
                critical: { bg: 'bg-red-100', text: 'text-red-700', bar: 'bg-red-500', label: 'Critical Risk' },
                high: { bg: 'bg-orange-100', text: 'text-orange-700', bar: 'bg-orange-500', label: 'High Risk' },
                medium: { bg: 'bg-amber-100', text: 'text-amber-700', bar: 'bg-amber-500', label: 'Medium Risk' },
                low: { bg: 'bg-green-100', text: 'text-green-700', bar: 'bg-green-500', label: 'Low Risk' }
            };
            return colors[level] || { bg: 'bg-gray-100', text: 'text-gray-600', bar: 'bg-gray-400', label: 'Unknown Risk' };
        }
    }">
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex items-center gap-2 flex-wrap">
                {{-- Export --}}
                <a href="{{ route('admin.ai.risk-rules.export', ['type' => $activeType]) }}"
                ...
                {{-- Reset --}}
                <form method="POST" action="{{ route('admin.ai.risk-rules.reset', ['type' => $activeType]) }}"
        ...
        {{-- AI Type Switcher --}}
        <div class="mb-6">
            <div class="flex items-center gap-1 bg-white border border-gray-200 rounded-xl shadow-sm p-1 w-fit">
                @foreach($aiTypes as $typeKey => $typeLabel)
                    @php
                        $isActive = $activeType === $typeKey;
                        $icon = $typeKey === 'login'
                            ? '<path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />'
                            : '<path stroke-linecap="round" stroke-linejoin="round" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />';
                        $accentColor = $typeKey === 'login' ? 'bg-violet-600 text-white shadow-sm' : 'bg-blue-600 text-white shadow-sm';
                    @endphp
                    <a href="{{ route('admin.ai.risk-rules.index', ['type' => $typeKey]) }}"
                       class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg transition-all
                              {{ $isActive ? $accentColor : 'text-gray-500 hover:text-gray-800 hover:bg-gray-50' }}">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            {!! $icon !!}
                        </svg>
                        {{ $typeLabel }}
                        <span class="ml-0.5 inline-flex items-center justify-center w-5 h-5 text-xs font-bold rounded-full
                                     {{ $isActive ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-600' }}">
                            {{ \App\Models\RiskRule::forType($typeKey)->count() }}
                        </span>
                    </a>
                @endforeach
            </div>
            <p class="mt-2 text-xs text-gray-400">
                @if($activeType === 'login')
                    Rules điều chỉnh scoring cho <strong>Login Risk AI</strong> — đánh giá rủi ro khi người dùng đăng nhập.
                @else
                    Rules điều chỉnh scoring cho <strong>Transaction Fraud AI</strong> — đánh giá rủi ro đơn hàng.
                @endif
            </p>
        </div>
        ...
        <div class="mb-6 p-4 bg-slate-50 border border-slate-200 rounded-xl flex items-start gap-3">
            ...
            <div class="text-sm text-slate-700">
                @if($activeType === 'login')
                    <strong>How scoring works:</strong> Each active rule contributes its <em>weight</em> to the login risk score.
                    A score ≥ 80 blocks access; ≥ 50 triggers MFA challenge. Higher weight = greater impact on the final decision.
                @else
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <template x-for="rule in filtered" :key="rule.key">
                            <tr x-transition:enter="transition ease-out duration-100"
                                x-transition:enter-start="opacity-0"
                                x-transition:enter-end="opacity-100"
                                :class="rule.active ? '' : 'opacity-50'"
                                class="hover:bg-gray-50/50 transition-colors">

                                {{-- Rule Key --}}
                                <td class="px-5 py-4">
                                    <div class="flex items-center gap-2">
                                        <div class="w-2 h-2 rounded-full flex-shrink-0" :class="rule.active ? 'bg-green-500' : 'bg-gray-300'"></div>
                                        <code class="font-mono text-xs bg-gray-100 text-gray-800 px-2 py-1 rounded" x-text="rule.key"></code>
                                    </div>
                                </td>

                                {{-- Description --}}
                                <td class="px-5 py-4 max-w-xs">
                                    <p class="text-sm text-gray-600 line-clamp-2" x-text="rule.desc"></p>
                                </td>

                                {{-- Risk Level --}}
                                <td class="px-5 py-4 text-center">
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold"
                                          :class="getRiskLevelInfo(rule.level).bg + ' ' + getRiskLevelInfo(rule.level).text"
                                          x-text="getRiskLevelInfo(rule.level).label">
                                    </span>
                                </td>
                                ...
                                <td class="px-5 py-4 text-center">
                                    <div class="flex flex-col items-center gap-1.5 min-w-[80px]">
                                        <span class="text-lg font-bold text-gray-900" x-text="rule.weight"></span>
                                        <div class="w-16 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                                            <div class="h-1.5 rounded-full"
                                                 :class="getRiskLevelInfo(rule.level).bar"
                                                 :style="'width: ' + Math.min(rule.weight, 100) + '%'"></div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                {{-- Status Toggle --}}
                                <td class="px-5 py-4 text-center">
                                    <form method="POST" :action="'{{ route('admin.ai.risk-rules.index') }}/' + rule.id + '/toggle'">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit"
                                            ...
                                            <span class="w-1.5 h-1.5 rounded-full" :class="rule.active ? 'bg-green-500' : 'bg-gray-400'"></span>
                                            <span x-text="rule.active ? 'Active' : 'Inactive'"></span>
                                        </button>
                                    </form>
                                </td>
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <button @click="openSimulateModal(rule)" type="button"
                                        ...
                                        @endif
                                        <a :href="'{{ route('admin.ai.risk-rules.index') }}/' + rule.id + '/edit'"
                        ...
                        {{-- Empty State --}}
                        <tr x-show="!Array.isArray(rules) || rules.length === 0" style="display: none;">
